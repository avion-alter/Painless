...
public class WeeklyRemainderActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_reminder_three);
        ...
        try {
            for (int i = 0; i < reminderResponce.getData().getPatientReminderData().size(); i++) {
                if (reminderResponce.getData().getPatientReminderData().get(i).getReminderType().equals("Weekly")){
                    ...
                    ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this, R.array.days_of_week, android.R.layout.simple_spinner_dropdown_item);
                    if (reminderResponce.getData().getPatientReminderData().get(i).getReminderWeekDay() != null) {
                        int spinnerPosition = adapter.getPosition(reminderResponce.getData().getPatientReminderData().get(i).getReminderWeekDay());
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_next:
                ...
                jsonObject.addProperty("reminder_type", "Weekly");
                jsonObject.addProperty("reminder_time", timePicker1.getHour() + ":" + timePicker1.getMinute() + ":00");
                jsonObject.addProperty("reminder_week_day", spinner.getSelectedItem().toString());
                jsonArray.add(jsonObject);



                saveDailyReminderViewModel.saveReminder(jsonArray.toString(), WeeklyRemainderActivity.this).observe(this, new Observer<SaveReminderResponse>() {
                    @Override
                    public void onChanged(@Nullable SaveReminderResponse reminderResponse) {
                        try {
                            if (reminderResponse.isStatus()) {
                                Toast.makeText(WeeklyRemainderActivity.this, "" + reminderResponse.getMessage(), Toast.LENGTH_SHORT).show();
                                Intent n = new Intent(WeeklyRemainderActivity.this, DashboardActivity.class);
