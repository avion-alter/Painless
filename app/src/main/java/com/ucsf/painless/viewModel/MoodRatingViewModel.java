...
public class MoodRatingViewModel extends ViewModel {
    ...
    public LiveData<IntensityResponse> saveMoodRating( String inrt_pain_in, String inrt_unpleasantness, Context context) {
        //if the list is null
        this.mContext=context;

        heroList = new MutableLiveData<IntensityResponse>();
        //we will load it asynchronously from server in this method
        saveMoodRating(inrt_pain_in,  inrt_unpleasantness);
        //finally we will return the list
        return heroList;
    }

    //This method is using Retrofit to get the JSON data from URL
    private void saveMoodRating( String inrt_pain_in,  String inrt_unpleasantness) {
        ...
        if(ApplicationTest.getInstance().connectionDetector.isConnectingToInternet()){
            ...
            pDialog.show();
            Log.e("Request",ApplicationTest.getInstance().sessionManager.getStringKey(SessionKeyConstant.KEY_user_id)+inrt_pain_in+inrt_unpleasantness);
            Call<IntensityResponse> call3 = apiInterface.saveMoodRating(ApplicationTest.getInstance().sessionManager.getStringKey(SessionKeyConstant.KEY_user_id),inrt_pain_in,inrt_unpleasantness);
