...
public class WeeklyFourViewModel extends ViewModel {
    ...
    private MutableLiveData<WeeklyFourResponse> heroList;
    private MutableLiveData<SaveWeeklyResponse> saveWeeklyResponseMutableLiveData;

    //we will call this method to get the data
    public LiveData<WeeklyFourResponse> getWeeklyFour(Context context) {
        //if the list is null
        this.mContext=context;

            heroList = new MutableLiveData<WeeklyFourResponse>();
    ...
    //we will call this method to get the data
    public LiveData<SaveWeeklyResponse> saveWeeklyFour(Context context,String weeklyOneJson,String report_id) {
        //if the list is null
        this.mContext=context;

        saveWeeklyResponseMutableLiveData = new MutableLiveData<SaveWeeklyResponse>();
        //we will load it asynchronously from server in this method
        saveWeeklyFour(weeklyOneJson,report_id);
        //finally we will return the list
        return saveWeeklyResponseMutableLiveData;
    }

    //This method is using Retrofit to get the JSON data from URL
    private void getWeeklyFour() {
        ...
        if(ApplicationTest.getInstance().connectionDetector.isConnectingToInternet()){
            ...
            pDialog.show();
            Call<WeeklyFourResponse> call3 = apiInterface.getWeeklyFour(ApplicationTest.getInstance().sessionManager.getStringKey(SessionKeyConstant.KEY_user_id));
            call3.enqueue(new Callback<WeeklyFourResponse>() {
                @Override
                public void onResponse(Call<WeeklyFourResponse> call, retrofit2.Response<WeeklyFourResponse> response) {
                ...
                @Override
                public void onFailure(Call<WeeklyFourResponse> call, Throwable t) {
                    pDialog.dismiss();
                    call.cancel();
                }
            });
    ...
    //This method is using Retrofit to get the JSON data from URL
    private void saveWeeklyFour(String weeklyOneJson,String report_id) {
        ...
        if(ApplicationTest.getInstance().connectionDetector.isConnectingToInternet()){
            ...
            pDialog.setCancelable(false);
            pDialog.show();
            Call<SaveWeeklyResponse> call3 = apiInterface.saveWeeklyFour(ApplicationTest.getInstance().sessionManager.getStringKey(SessionKeyConstant.KEY_user_id),report_id,weeklyOneJson);
